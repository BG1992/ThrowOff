...
combs_right = [()]
for k in range(1,5):
    for comb in combinations_with_replacement(sec5, k):
        to_check = True
        for c in comb:
...
states_computed = list(filter(lambda x: x[0][1][0] != x[0][1][1] and x[0][1][1] != x[0][1][2]
                              and x[0][1][2] != x[0][1][3], states_computed))
